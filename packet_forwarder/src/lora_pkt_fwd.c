...
static int  mqtt_sub_port = 1883;
static char mqtt_sub_user[64] = "";
static char mqtt_sub_pass[64] = "";
static char mqtt_sub_topic[128] = "";
static uint8_t forge_app_s_key[16]; 
static uint8_t forge_nwk_s_key[16]; 
static bool keys_configured = false; 

static struct mosquitto *mosq = NULL;
static char mqtt_topic_up[128];
static char gateway_id_str[32];
...
static pthread_mutex_t mx_crypto = PTHREAD_MUTEX_INITIALIZER;
static bool xtal_correct_ok = false; /* set true when XTAL correction is stable enough */
static double xtal_correct = 1.0;

static pthread_mutex_t mx_udp_send = PTHREAD_MUTEX_INITIALIZER;
static uint8_t forged_buff_up[TX_BUFF_SIZE];
...
void thread_mqtt_subscriber(void);
...
int hex_to_bin(const char *hex_str, uint8_t *bin_buf, size_t bin_size) {
	size_t hex_len = strlen(hex_str);
	if (hex_len % 2 != 0 || (hex_len / 2) > bin_size) {
		return -1;
	}
	for (size_t i = 0; i < hex_len; i += 2) {
		sscanf(hex_str + i, "%2hhx", &bin_buf[i / 2]);
	}
	return hex_len / 2;
}

// 新的函数：加密并构造一个完整的 LoRaWAN PHY Payload
int encrypt_and_forge_packet(const uint8_t* plaintext, int plen, uint8_t fport, uint32_t dev_addr, uint32_t fcnt, struct lgw_pkt_rx_s* forged_pkt) {
    uint8_t phy_payload[256];
    const int LORAWAN_HEADER_MIC_SIZE = 13;

    if (plen > (sizeof(phy_payload) - LORAWAN_HEADER_MIC_SIZE)) {
        MSG("ERROR: [forge] plaintext payload size (%d) is too large for LoRaWAN frame. Max allowed is %d.\n", plen, (int)(sizeof(phy_payload) - LORAWAN_HEADER_MIC_SIZE));
        return -1;
    }

    if (!keys_configured) {
        MSG("ERROR: [forge] AppSKey and NwkSKey are not configured in JSON file.\n");
        return -1;
    }

    int phy_idx = 0;
    int mac_payload_len = 0;
    uint32_t mic = 0;

    // 1. 构造 MAC 载荷 (MACPayload)
    phy_payload[phy_idx++] = 0x40;
    phy_payload[phy_idx++] = dev_addr & 0xFF;
    phy_payload[phy_idx++] = (dev_addr >> 8) & 0xFF;
    phy_payload[phy_idx++] = (dev_addr >> 16) & 0xFF;
    phy_payload[phy_idx++] = (dev_addr >> 24) & 0xFF;
    phy_payload[phy_idx++] = 0x00;
    phy_payload[phy_idx++] = fcnt & 0xFF;
    phy_payload[phy_idx++] = (fcnt >> 8) & 0xFF;
    phy_payload[phy_idx++] = fport;
    
    // 【关键修正】在调用非线程安全的加密函数前后加锁和解锁
    pthread_mutex_lock(&mx_crypto);
    ...
    // 2. 计算 MIC
    LoRaMacComputeMic(phy_payload, mac_payload_len, forge_nwk_s_key, dev_addr, 0, fcnt, &mic);
    ...
    phy_payload[phy_idx++] = (mic >> 8) & 0xFF;
    phy_payload[phy_idx++] = (mic >> 16) & 0xFF;
    phy_payload[phy_idx++] = (mic >> 24) & 0xFF;

    // 4. 填充最终的 forged_pkt 结构体
    memset(forged_pkt, 0, sizeof(struct lgw_pkt_rx_s));
    forged_pkt->size = phy_idx;
    memcpy(forged_pkt->payload, phy_payload, phy_idx);
    
    // 填充射频元数据
    forged_pkt->status = STAT_CRC_OK;
    forged_pkt->freq_hz = 486700000;
    forged_pkt->if_chain = 0;
    forged_pkt->rf_chain = 0;
    forged_pkt->modulation = MOD_LORA;
    forged_pkt->bandwidth = BW_125KHZ;
    forged_pkt->datarate = DR_LORA_SF7;
    forged_pkt->coderate = CR_LORA_4_5;
    forged_pkt->rssic = -75.0;
    forged_pkt->rssis = -77.0;
    forged_pkt->snr = 10.5;
...
void send_forged_packet_udp_independent(struct lgw_pkt_rx_s *p) {
    ...
    forged_buff_up[3] = PKT_PUSH_DATA;
    *(uint32_t *)(forged_buff_up + 4) = net_mac_h;
    *(uint32_t *)(forged_buff_up + 8) = net_mac_l;
    ...
    memcpy((void *)(forged_buff_up + buff_index), (void *)",\"stat\":1", 9);
    buff_index += 9;

    memcpy((void *)(forged_buff_up + buff_index), (void *)",\"modu\":\"LORA\"", 14);
    buff_index += 14;
    
    // 修正SF值的显示
    if (p->datarate >= DR_LORA_SF5 && p->datarate <= DR_LORA_SF12) {
        j = snprintf((char *)(forged_buff_up + buff_index), TX_BUFF_SIZE-buff_index, ",\"datr\":\"SF%dBW125\"", p->datarate - DR_LORA_SF7 + 7);
    } else {
        j = snprintf((char *)(forged_buff_up + buff_index), TX_BUFF_SIZE-buff_index, ",\"datr\":\"SF%dBW125\"", 7); // Default to SF7
    }
    buff_index += j;
    
    memcpy((void *)(forged_buff_up + buff_index), (void *)",\"codr\":\"4/5\"", 13);
    buff_index += 13;

    j = snprintf((char *)(forged_buff_up + buff_index), TX_BUFF_SIZE-buff_index, ",\"lsnr\":%.1f,\"rssi\":%.0f", p->snr, p->rssic);
    buff_index += j;

    j = snprintf((char *)(forged_buff_up + buff_index), TX_BUFF_SIZE-buff_index, ",\"size\":%u", p->size);
    buff_index += j;
    memcpy((void *)(forged_buff_up + buff_index), (void *)",\"data\":\"", 9);
    buff_index += 9;
    j = bin_to_b64(p->payload, p->size, (char *)(forged_buff_up + buff_index), 341);
    buff_index += j;
    forged_buff_up[buff_index] = '"';
    ++buff_index;

    memcpy((void *)(forged_buff_up + buff_index), (void *)"}]}", 3);
    buff_index += 3;
...
void on_message_forge_uplink(struct mosquitto *mosq, void *obj, const struct mosquitto_message *message) {
    MSG("INFO: [mqtt_sub] Received message on topic %s\n", message->topic);

    JSON_Value *root_val = json_parse_string((const char *)message->payload);
    if (root_val == NULL) {
        MSG("WARNING: [mqtt_sub] Invalid JSON in MQTT message\n");
        return;
    }
    JSON_Object *root_obj = json_value_get_object(root_val);

    const char *data_str = json_object_get_string(root_obj, "data");
    const char *dev_addr_str = json_object_get_string(root_obj, "devAddr");
    double fport = json_object_get_number(root_obj, "fPort");
    
    if (data_str == NULL || dev_addr_str == NULL || fport == 0) {
        MSG("WARNING: [mqtt_sub] JSON missing 'data', 'devAddr', or 'fPort'\n");
        json_value_free(root_val);
        return;
    }

    uint8_t plaintext_buf[256];
    int plaintext_size = snprintf((char*)plaintext_buf, sizeof(plaintext_buf), "%s", data_str);

    if (plaintext_size >= sizeof(plaintext_buf)) {
        plaintext_size = sizeof(plaintext_buf) - 1;
        MSG("WARNING: [mqtt_sub] MQTT data payload was truncated to %d bytes.\n", plaintext_size);
    }

    uint32_t dev_addr;
    sscanf(dev_addr_str, "%x", &dev_addr);
    
    static uint32_t forged_fcnt = 0;

    struct lgw_pkt_rx_s forged_pkt;
...
void thread_mqtt_subscriber(void) {
    ...
    MSG("INFO: [mqtt_sub] Starting MQTT subscriber thread...\n");

    mosquitto_lib_init();
    mosq = mosquitto_new(NULL, true, NULL);
    if (!mosq) {
        MSG("ERROR: [mqtt_sub] Failed to create Mosquitto client.\n");
        return;
    }

    mosquitto_message_callback_set(mosq, on_message_forge_uplink);
	...
	if (tls_rc != MOSQ_ERR_SUCCESS) {
		MSG("ERROR: [mqtt_sub] Failed to set TLS options: %s\n", mosquitto_strerror(tls_rc));
		mosquitto_destroy(mosq);
		mosquitto_lib_cleanup();
		return;
	}
	MSG("INFO: [mqtt_sub] TLS is enabled with hardcoded certificate path.\n");
    
    if (strlen(mqtt_sub_user) > 0) {
        mosquitto_username_pw_set(mosq, mqtt_sub_user, mqtt_sub_pass);
    }

    rc = mosquitto_connect(mosq, mqtt_sub_host, mqtt_sub_port, 60);
    if (rc != MOSQ_ERR_SUCCESS) {
        MSG("ERROR: [mqtt_sub] Unable to connect to MQTT broker: %s\n", mosquitto_strerror(rc));
        mosquitto_destroy(mosq);
        mosquitto_lib_cleanup();
        return;
    }

    rc = mosquitto_subscribe(mosq, NULL, mqtt_sub_topic, 0);
     if (rc != MOSQ_ERR_SUCCESS) {
        MSG("ERROR: [mqtt_sub] Subscription failed: %s\n", mosquitto_strerror(rc));
        mosquitto_destroy(mosq);
        mosquitto_lib_cleanup();
        return;
    } else {
...
void mqtt_initialize(const char* host, int port, const char* user, const char* pass, uint64_t gateway_id) {
	int rc;

	MSG("INFO: Initializing Mosquitto library...\n");
	mosquitto_lib_init();

	mosq = mosquitto_new(NULL, true, NULL);
	if (!mosq) {
		MSG("ERROR: Failed to create Mosquitto client instance.\n");
		return;
	}

	if (user && pass && strlen(user) > 0) {
		mosquitto_username_pw_set(mosq, user, pass);
		MSG("INFO: MQTT authentication configured for user '%s'.\n", user);
	}

	MSG("INFO: Connecting to MQTT broker at %s:%d\n", host, port);
	rc = mosquitto_connect(mosq, host, port, 60);
	if (rc != MOSQ_ERR_SUCCESS) {
		MSG("ERROR: Unable to connect to MQTT broker: %s\n", mosquitto_strerror(rc));
		mosquitto_destroy(mosq);
		mosq = NULL;
		return;
	}
	...
	if (rc != MOSQ_ERR_SUCCESS) {
		MSG("ERROR: Unable to start Mosquitto loop: %s\n", mosquitto_strerror(rc));
		mosquitto_destroy(mosq);
		mosq = NULL;
		return;
	}
											
	snprintf(gateway_id_str, sizeof(gateway_id_str), "%016llx", (long long unsigned int)gateway_id);
	snprintf(mqtt_topic_up, sizeof(mqtt_topic_up), "gateway/%s/event/up", gateway_id_str);
		
	printf("gateway_id is %s\n", gateway_id_str);
	MSG("INFO: MQTT client initialized. Uplink topic: %s\n", mqtt_topic_up);
}
											
void mqtt_cleanup() {
	if (mosq) {
		MSG("INFO: Cleaning up MQTT client...\n");
...
void send_data_via_mqtt(struct lgw_pkt_rx_s *p) {
	//if mosq not start, return
	if (!mosq) {
	return;
	}

	char payload_b64[1024];
	char mqtt_payload[2048];
	char codr_str[16];
	int bw_khz;
	int rc;

	bin_to_b64(p->payload, p->size, payload_b64, sizeof(payload_b64));
								
	switch(p->bandwidth) {
		case BW_125KHZ: bw_khz = 125000; break;
		case BW_250KHZ: bw_khz = 250000; break;
		case BW_500KHZ: bw_khz = 500000; break;
		default: bw_khz = 0; break;
	}
								
	switch(p->coderate) {
		case CR_LORA_4_5: strcpy(codr_str, "CR_4_5"); break;
		case CR_LORA_4_6: strcpy(codr_str, "CR_4_6"); break;
		case CR_LORA_4_7: strcpy(codr_str, "CR_4_7"); break;
		case CR_LORA_4_8: strcpy(codr_str, "CR_4_8"); break;
		default: strcpy(codr_str, "CR_UNDEFINED"); break;
	}
	...
	rc = mosquitto_publish(mosq, NULL, mqtt_topic_up, strlen(mqtt_payload), mqtt_payload, 1, false);
	if (rc != MOSQ_ERR_SUCCESS) {
		MSG("ERROR: Failed to publish MQTT message: %s\n", mosquitto_strerror(rc));
		} else {
		printf("gateway_id is %s\n", gateway_id_str);
		printf("INFO: MQTT message published successfully.\n");
	}
}

void send_stats_via_mqtt(const char* status_payload) {
	if (!mosq) {
		return;
	}

	char mqtt_topic_stats[128];
	int rc;

	snprintf(mqtt_topic_stats, sizeof(mqtt_topic_stats), "gateway/%s/event/stats", gateway_id_str);

	rc = mosquitto_publish(mosq, NULL, mqtt_topic_stats, strlen(status_payload), status_payload, 1, false);
	if (rc != MOSQ_ERR_SUCCESS) {
		MSG("ERROR: Failed to publish MQTT stats message: %s\n", mosquitto_strerror(rc));
	} else {
		MSG("INFO: MQTT stats message published successfully.\n");
	}
}
...
static int parse_gateway_configuration(const char * conf_file) {
	...
	/*  MQTT Subscriber for uplink simulation (optional) */
	JSON_Object *mqtt_sub_conf = json_object_get_object(conf_obj, "mqtt_subscriber");
	if (mqtt_sub_conf != NULL) {
		val = json_object_get_value(mqtt_sub_conf, "enable");
		if (json_value_get_type(val) == JSONBoolean) {
			mqtt_sub_enabled = (bool)json_value_get_boolean(val);
		}

		if (mqtt_sub_enabled) {
			str = json_object_get_string(mqtt_sub_conf, "server_address");
			if (str != NULL) strncpy(mqtt_sub_host, str, sizeof(mqtt_sub_host) - 1);

			val = json_object_get_value(mqtt_sub_conf, "server_port");
			if (val != NULL) mqtt_sub_port = (int)json_value_get_number(val);

			str = json_object_get_string(mqtt_sub_conf, "user");
			if (str != NULL) strncpy(mqtt_sub_user, str, sizeof(mqtt_sub_user) - 1);
																													            
			str = json_object_get_string(mqtt_sub_conf, "password");
			if (str != NULL) strncpy(mqtt_sub_pass, str, sizeof(mqtt_sub_pass) - 1);

			str = json_object_get_string(mqtt_sub_conf, "topic");
			if (str != NULL) strncpy(mqtt_sub_topic, str, sizeof(mqtt_sub_topic) - 1);

			const char* appskey_str = json_object_get_string(mqtt_sub_conf, "appSKey");
			const char* nwkskey_str = json_object_get_string(mqtt_sub_conf, "nwkSKey");

			if (appskey_str && nwkskey_str) {
				hex_to_bin(appskey_str, forge_app_s_key, 16);
				hex_to_bin(nwkskey_str, forge_nwk_s_key, 16);
				keys_configured = true;
				MSG("INFO: MQTT subscriber keys (AppSKey, NwkSKey) loaded.\n");
			} else {
				MSG("WARNING: MQTT subscriber enabled, but 'appSKey' or 'nwkSKey' are missing in config.\n");
			}
									            
			MSG("INFO: MQTT subscriber for uplink simulation is enabled.\n");
		}
	}
...
int main(int argc, char ** argv)
{
	...
	mqtt_config_t chirp_cfg;
	memset(&chirp_cfg, 0, sizeof(chirp_cfg));
#if 0
	if (pares_mqtt_control(conf_fname) == 0)
	{
		if (parse_mqtt_configuration(conf_fname, &chirp_cfg, 2) != 0)
		{
			printf("get chirp stack config failure\n");
		}
		else
		{
			mqtt_initialize(chirp_cfg.host, chirp_cfg.port, NULL, NULL, lgwm);
		}
	}
#endif
	...
	if (mqtt_sub_enabled) {
		...
		if (i != 0) {
			MSG("ERROR: [main] impossible to create MQTT subscriber thread\n");
			exit(EXIT_FAILURE);
		}
	}
    ...
    while (!exit_sig && !quit_sig) {
        ...
        report_ready = true;
		//send_stats_via_mqtt(status_report);
...
static void gps_process_sync(void) {
    ...
    pthread_mutex_unlock(&mx_concent);
    if (i != LGW_HAL_SUCCESS) {
        printf("WARNING: [gps] failed to read concentrator timestamp\n");
        return;
    }
...
void thread_valid(void) {
    ...
    while (!exit_sig && !quit_sig) {
		...
		//printf("当前系统时间 (time(NULL)): %s", ctime(&current_time));
		//printf("GPS 参考时间 (systime):    %s", ctime(&time_reference_gps.systime));
